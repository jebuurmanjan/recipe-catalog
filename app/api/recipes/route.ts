...
export async function POST(req: NextRequest) {
  ...
  const { title, description, ingredients, steps, image_url, source_url, tagIds, categoryIds, is_concept, servings, serves, prep_time } = body
  ...
  let { data: recipe, error } = await db
    .from('recipes')
    .insert({ title, slug, description, ingredients, steps, image_url, source_url, is_concept: is_concept ?? false, servings: servings ?? null, serves: serves ?? null, prep_time: prep_time ?? null, user_id: user.id })
    .select()
    .single()
