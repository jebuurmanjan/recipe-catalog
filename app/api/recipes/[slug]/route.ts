...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  const { data: recipe, error } = await db
    .from('recipes')
    .update({ title, slug: newSlug, description, ingredients, steps, image_url, source_url, is_concept: is_concept ?? false, servings: servings ?? null, serves: serves ?? null, prep_time: prep_time ?? null })
    .eq('id', existing.id)
    .select()
    .single()
